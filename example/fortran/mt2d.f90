...
program main
   ...
   real(kind=8) vartem, dx, length, delta, area, vol, is, stenload1, stenload2, thick, theta, width
   real(kind=8) tempcload1, dt, totime, ctime, idist, fac, radij, dflux1, dforce1, dforce2
   real(kind=8) pi, ta, tb, ma, cc, ev
   real(kind=8) scrt, scrm, scr, scx, scy, coordx, coordy

   real(kind=8) coord(totnode,2), pflux(totnode,1), tempddens(totnode,1), stendens(totnode,2), bforce(totnode,2)
   real(kind=8) fncsth(totnode,1), fncstm(totnode,2), tem(totnode,1), disp(totnode,2)
   real(kind=8) vel(totnode,2), acc(totnode,2), pforce(totnode,2), hs(totnode,1)
